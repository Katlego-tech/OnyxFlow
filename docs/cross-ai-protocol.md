...
Currently in play:
> - Katlego = Claude (entry point [CLAUDE.md](../CLAUDE.md))
>
> One person and one tool today — but more than one Claude session at a time is normal, and two
> sessions collide exactly like two people do. The rules below are identical **no matter how many
> tools are in play or which ones**; that's the point of routing everything through AGENTS.md
> instead of writing per-tool rules. A second tool joining needs only its own entry-point file
> (copy `AI_ENTRYPOINT.template.md`) and a row in AGENTS.md §1.
