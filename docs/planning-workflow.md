...
`OnyxFlow` is planned with plain, self-driven documents — no external orchestrator required.
Four living documents plus a folder of design docs are all you need.

OnyxFlow runs the **plain** path described here — no Spec-Kit, no `specify` CLI, no external
orchestrator. "The heavier alternative" below is documented for completeness; the two are not meant
to run at once, and adopting it would be a PLAN.md change, not a personal preference.
