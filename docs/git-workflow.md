...
The version actually installed at [`.githooks/pre-push`](../.githooks/pre-push) is customized for
this repo: it prefers `.venv/bin/python` over whatever is on `PATH`, runs `pytest` once from the
root (there is one Python package, not a `services/*` tree), and checks for `node_modules` **inside**
each `apps/*` workspace rather than at the repo root — the template's root-level check silently
skipped the client's suite, which is worse than no gate at all. Missing tooling fails loudly with
the command that fixes it, rather than passing quietly.
